...
2. **Execution cycle** — Azure timer every 15 seconds. It is a short-lived, reentrant, deterministic loop. It never waits for Brain. It advances durable state and returns.
...
1. **EOD forced close**. If the configured EOD close condition applies to an OPEN position, close it, persist the resulting position state, log it, and stop processing that symbol for this cycle.
...
4. **Read latest market data and execute the symbol**. There is no separate generic “market freshness gate” and no separate “execute active plan” stage. One execution function handles both OPEN and FLAT states:
   ...
   - FLAT: execute a valid accepted OPEN instruction if its condition is met.
...
The durable Position record is executable truth. Once a plan is accepted, stop-loss/take-profit values are copied into Position. Position management must not depend on GitHub or Brain remaining available.
...
V1 supports at most one optional ADD instruction and one optional REDUCE instruction. Total target quantity is limited to 0–6 micros per symbol. No dynamic risk model, portfolio VaR, pyramiding tree, or multi-target tree is in v1.
...
Each 15-minute scheduled Brain run reads exactly these logical inputs per symbol:
...
5. a separately editable strategy prompt document.
...
- EOD forced-close local time and timezone;
...
Dashboard/watchdog reads these logs and durable state directly. Dashboard output never controls trading.
