...
def execute(document, *, risk_context=None, ledger=None, broker=None, reservation_hash=None):
    return execute_reserved_plan(
        document,
        event_id=EVENT_ID,
        reservation_plan_hash=reservation_hash or canonical_plan_hash(document),
        context=risk_context or context(),
        risk_gateway=RiskGateway(),
        broker=broker or DeterministicPaperBroker(),
...
def test_no_trade_and_hold_are_terminal_without_order_or_fill(decision):
    ...
    result = execute(plan(decision, position_action=None), broker=broker)

    assert result.terminal is True
    assert result.status == "NO_EXECUTION"
    assert result.reason_code == f"PLAN_{decision}"
    assert result.order is None
    assert result.fill is None
    assert broker.calls == 0
    assert [receipt["stage"] for receipt in result.receipts] == ["EXECUTOR_RECEIVED", "COMPLETED"]


@pytest.mark.parametrize("decision", ["LONG", "SHORT"])
def test_directional_plan_requires_explicit_protective_stop(decision):
    document = plan(decision, position_action={"quantity": 1})
    result = execute(document)

    assert result.status == "REJECTED"
    assert result.reason_code == "MISSING_PROTECTIVE_STOP"
    assert result.order is None
    assert result.fill is None
...
@pytest.mark.parametrize(
    "risk_context,reason",
    [
        (context(market_state=market(feed_as_of=NOW - timedelta(seconds=91))), "STALE_FEED"),
        (context(session_open=False), "SESSION_CLOSED"),
        (context(kill_switch=True), "KILL_SWITCH_ACTIVE"),
        (
            context(account_state=account(daily_realized_pnl_usd=Decimal("-600.00"))),
            "DAILY_LOSS_LIMIT_REACHED",
        ),
        (context(account_state=account(consecutive_failures=3)), "CONSECUTIVE_FAILURE_LIMIT_REACHED"),
...
def test_directional_plan_fails_closed_on_every_required_safety_gate(risk_context, reason):
    broker = CountingPaperBroker()
    result = execute(plan(), risk_context=risk_context, broker=broker)
...
@pytest.mark.parametrize(
    ...
    [
        account(mode="LIVE"),
        account(starting_equity_usd=Decimal("100000.00")),
    ],
)
def test_executor_accepts_only_the_authorized_50k_paper_account(account_state):
    result = execute(plan(), risk_context=context(account_state=account_state))
    assert result.status == "REJECTED"
    assert result.reason_code == "UNAUTHORIZED_ACCOUNT"
...
def test_duplicate_plan_id_cannot_create_a_second_order_or_fill():
    ...
    first = execute(document, ledger=ledger, broker=broker)
    second = execute(document, ledger=ledger, broker=broker)

    assert first.status == "FILLED"
    assert second == first
    assert broker.calls == 1
    assert len([receipt for receipt in first.receipts if receipt["stage"] == "PAPER_ORDERED"]) == 1
    assert len([receipt for receipt in first.receipts if receipt["stage"] == "PAPER_FILLED_OR_REJECTED"]) == 1


def test_reservation_hash_must_match_the_exact_immutable_plan():
    broker = CountingPaperBroker()
    result = execute(plan(), broker=broker, reservation_hash="0" * 64)

    assert result.status == "REJECTED"
    assert result.reason_code == "RESERVATION_HASH_MISMATCH"
    assert broker.calls == 0
...
def test_paper_fill_is_deterministic_conservative_and_uses_next_bar_only():
    ...
    second = execute(document, ledger=ExecutionLedger())

    assert first.fill == second.fill
    assert first.fill.price == Decimal("6000.25")
    assert first.fill.occurred_at == NOW
    assert first.fill.price != market().next_bar_open
...
def test_receipts_are_deterministic_and_correlated_by_event_and_plan():
    ...
    assert all(receipt["event_id"] == EVENT_ID for receipt in result.receipts)
    assert all(receipt["plan_id"] == EVENT_ID for receipt in result.receipts)
    assert len({receipt["receipt_id"] for receipt in result.receipts}) == len(result.receipts)


def test_trade_risk_and_quantity_limits_fail_closed():
    too_wide_stop = plan(position_action={"quantity": 1, "protective_stop": {"price": "5969.00"}})
    too_many_contracts = plan(position_action={"quantity": 2, "protective_stop": {"price": "5990.00"}})

    assert execute(too_wide_stop).reason_code == "MAX_TRADE_RISK_EXCEEDED"
    assert execute(too_many_contracts).reason_code == "POSITION_LIMIT_EXCEEDED"


def test_market_provenance_and_three_closed_bar_start_gate_fail_closed():
    bad_provenance = context(market_state=market(data_class="TEST"))
    insufficient_history = context(market_state=market(consecutive_closed_bars=2))

    assert execute(plan(), risk_context=bad_provenance).reason_code == "UNTRUSTED_FEED"
    assert execute(plan(), risk_context=insufficient_history).reason_code == "INSUFFICIENT_FEED_HISTORY"
