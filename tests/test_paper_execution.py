...
def test_directional_plan_fails_closed_on_every_required_safety_gate(risk_context,reason):
    broker=CountingPaperBroker(); result=execute(plan(),risk_context=risk_context,broker=broker)
...
def test_executor_accepts_only_the_authorized_50k_paper_account(account_state):
    result=execute(plan(),risk_context=context(account_state=account_state)); assert result.status=="REJECTED"; assert result.reason_code=="UNAUTHORIZED_ACCOUNT"


def test_duplicate_plan_id_cannot_create_a_second_order_or_fill():
    ledger=ExecutionLedger(); broker=CountingPaperBroker(); document=plan(); first=execute(document,ledger=ledger,broker=broker); second=execute(document,ledger=ledger,broker=broker)
...
def test_paper_fill_is_deterministic_conservative_and_uses_next_bar_only():
    document=plan(); first=execute(document); second=execute(document,ledger=ExecutionLedger())
...
def test_entry_fill_keeps_open_position_nonterminal_until_exit_lifecycle_completes():
    result=execute(plan()); assert result.position is not None; assert result.position.status=="OPEN"; assert result.terminal is False; assert result.reason_code=="PAPER_ENTRY_FILLED_POSITION_OPEN"
...
def test_trade_risk_and_quantity_limits_fail_closed():
    assert execute(plan(position_action={"quantity":1,"protective_stop":{"price":"5969.00"}})).reason_code=="MAX_TRADE_RISK_EXCEEDED"
    assert execute(plan(position_action={"quantity":2,"protective_stop":{"price":"5990.00"}})).reason_code=="POSITION_LIMIT_EXCEEDED"
